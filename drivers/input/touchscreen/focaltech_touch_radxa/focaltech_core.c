...
*****************************************************************************/
#define FTS_DRIVER_NAME                     "raspi_ts"
...
/*****************************************************************************
*  Name: fts_reset_proc_r
*  Brief: Execute reset operation
*  Input: hdelayms - delay time unit:ms
*  Output:
*  Return:
*****************************************************************************/
int fts_reset_proc_r(int hdelayms)
{
    FTS_FUNC_ENTER();
    if(gpio_is_valid(fts_data_r->pdata->reset_gpio)) {
        gpio_direction_output(fts_data_r->pdata->reset_gpio, 0);
        msleep(5);
        gpio_direction_output(fts_data_r->pdata->reset_gpio, 1);
        if (hdelayms) {
            msleep(hdelayms);
        }
    }
...
*****************************************************************************/
int fts_wait_tp_to_valid_r(struct i2c_client *client)
{
    int ret = 0;
    int cnt = 0;
    u8 reg_value = 0;
    u8 chip_id = fts_data_r->ic_info.ids.chip_idh;
	fts_reset_proc_r(200);
    do {
        ret = fts_i2c_read_r_reg(client, FTS_REG_CHIP_ID, &reg_value);
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    do {
        ret = fts_i2c_read_r_reg(client, FTS_REG_CHIP_ID, &chip_id[0]);
        ret1 = fts_i2c_read_r_reg(client, FTS_REG_CHIP_ID2, &chip_id[1]);
    ...
    if ((cnt * INTERVAL_READ_REG) >= TIMEOUT_READ_REG) {
        ...
        ret = fts_i2c_write_r(client, id_cmd, 2);
        if (ret < 0) {
        ...
        ret = fts_i2c_read_r(client, id_cmd, id_cmd_len, chip_id, 2);
...
/*****************************************************************************
*  Name: fts_tp_state_recovery_r
*  Brief: Need execute this function when reset
*  Input:
*  Output:
*  Return:
*****************************************************************************/
void fts_tp_state_recovery_r(struct i2c_client *client)
{
    FTS_FUNC_ENTER();
    /* wait tp stable */
    fts_wait_tp_to_valid_r(client);
    /* recover TP charger state 0x8B */
    /* recover TP glove state 0xC0 */
    /* recover TP cover state 0xC1 */
    fts_ex_mode_recovery_r(client);
...
/*****************************************************************************
*  Name: fts_irq_disable_r
*  Brief: disable irq
*  Input:
*  Output:
*  Return:
*****************************************************************************/
void fts_irq_disable_r(void)
{
    unsigned long irqflags;

    FTS_FUNC_ENTER();
    spin_lock_irqsave(&fts_data_r->irq_lock, irqflags);

    if (!fts_data_r->irq_disabled) {
        disable_irq_nosync(fts_data_r->irq);
        fts_data_r->irq_disabled = true;
    }

    spin_unlock_irqrestore(&fts_data_r->irq_lock, irqflags);
    FTS_FUNC_EXIT();
}

/*****************************************************************************
*  Name: fts_irq_enable_r
*  Brief: enable irq
*  Input:
*  Output:
*  Return:
*****************************************************************************/
void fts_irq_enable_r(void)
{
    ...
    if (fts_data_r->irq_disabled) {
        enable_irq(fts_data_r->irq);
        fts_data_r->irq_disabled = false;
    }
...
static void fts_show_touch_buffer(u8 *buf, int point_num)
{
    ...
    if (len > (fts_data_r->pnt_buf_size - 3)) {
        len = fts_data_r->pnt_buf_size - 3;
    } else if (len == 0) {
        len += FTS_ONE_TCH_LEN;
    }
...
static void fts_release_all_finger(void)
{
    struct input_dev *input_dev = fts_data_r->input_dev;
#if FTS_MT_PROTOCOL_B_EN
    u32 finger_count = 0;
#endif

    FTS_FUNC_ENTER();
    mutex_lock(&fts_data_r->report_mutex);
#if FTS_MT_PROTOCOL_B_EN
    for (finger_count = 0; finger_count < fts_data_r->pdata->max_touch_number; finger_count++) {
    ...
    mutex_unlock(&fts_data_r->report_mutex);
    FTS_FUNC_EXIT();
}
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    ret = fts_i2c_read_r(data->client, buf, 1, buf, data->pnt_buf_size);
    if (ret < 0) {
    ...
    if (data->ic_info.is_incell) {
        if ((data->point_num == 0x0F) && (buf[1] == 0xFF) && (buf[2] == 0xFF)
            ...
            fts_tp_state_recovery_r(client);
            return -EIO;
        }
    }
                                ...
                                unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data && event == FB_EVENT_BLANK &&
        fts_data_r && fts_data_r->client) {
        blank = evdata->data;
        if (*blank == FB_BLANK_UNBLANK)
            queue_work(fts_data_r->ts_workqueue, &fts_data_r->resume_work);
        else if (*blank == FB_BLANK_POWERDOWN)
            fts_ts_suspend(&fts_data_r->client->dev);
    }
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!ts_data) {
        FTS_ERROR("Failed to allocate memory for fts_data_r");
        return -ENOMEM;
    }
...
#else
    /* TP enter sleep mode */
    ret = fts_i2c_write_r_reg(ts_data->client, FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP_VALUE);
    if (ret < 0)
...
static int fts_ts_resume(struct device *dev)
{
    ...
    fts_tp_state_recovery_r(ts_data->client);
