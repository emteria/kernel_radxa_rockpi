...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    int ret = 0;
    char tmp[25];
    struct fts_ts_data *ts_data = fts_data_r;
    ...
    case PROC_READ_REGISTER:
        writelen = 1;
        ret = fts_i2c_write_r(client, writebuf + 1, writelen);
        if (ret < 0) {
...
static ssize_t fts_debug_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_READ_REGISTER:
        num_read_chars = 1;
        ret = fts_i2c_read_r(client, NULL, 0, buf, num_read_chars);
        if (ret < 0) {
#if FTS_ESDCHECK_EN
...
static ssize_t fts_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_esdcheck_proc_busy(1);
#endif
    if (fts_i2c_read_r_reg(client, FTS_REG_FW_VER, &fwver) < 0) {
    ...
    if (rw_op.len < 0) {
        ...
        if (RWREG_OP_READ == rw_op.type) {
            if (rw_op.len == 1) {
                u8 reg, val;
                reg = rw_op.reg & 0xFF;
                rw_op.res = fts_i2c_read_r_reg(client, reg, &val);
                rw_op.val = val;
            } else {
                char reg;
                reg = rw_op.reg & 0xFF;

                rw_op.res = fts_i2c_read_r(client, &reg, 1, rw_op.opbuf, rw_op.len);
            }
            ...
            if (rw_op.len == 1) {
                ...
                reg = rw_op.reg & 0xFF;
                val = rw_op.val & 0xFF;
                rw_op.res = fts_i2c_write_r_reg(client, reg, val);
            } else {
                rw_op.res = fts_i2c_write_r(client, rw_op.opbuf, rw_op.len);
            }
            if (rw_op.res < 0) {
...
/*
 * fts_upgrade_bin_r interface
 */
static ssize_t fts_fwupgrade_rbin_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return -EPERM;
}

static ssize_t fts_fwupgrade_rbin_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    fts_upgrade_bin_r(client, fwname, 0);
...
static ssize_t fts_fwforceupg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    fts_upgrade_bin_r(client, fwname, 1);
...
static ssize_t fts_dumpreg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_esdcheck_proc_busy(1);
#endif
    fts_i2c_read_r_reg(client, FTS_REG_POWER_MODE, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Power Mode:0x%02x\n", val);

    fts_i2c_read_r_reg(client, FTS_REG_FW_VER, &val);
    count += snprintf(buf + count, PAGE_SIZE, "FW Ver:0x%02x\n", val);

    fts_i2c_read_r_reg(client, FTS_REG_LIC_VER, &val);
    count += snprintf(buf + count, PAGE_SIZE, "LCD Initcode Ver:0x%02x\n", val);

    fts_i2c_read_r_reg(client, FTS_REG_IDE_PARA_VER_ID, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Param Ver:0x%02x\n", val);

    fts_i2c_read_r_reg(client, FTS_REG_IDE_PARA_STATUS, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Param status:0x%02x\n", val);

    fts_i2c_read_r_reg(client, FTS_REG_VENDOR_ID, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Vendor ID:0x%02x\n", val);

    fts_i2c_read_r_reg(client, FTS_REG_LCD_BUSY_NUM, &val);
    count += snprintf(buf + count, PAGE_SIZE, "LCD Busy Number:0x%02x\n", val);

    fts_i2c_read_r_reg(client, FTS_REG_GESTURE_EN, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Gesture Mode:0x%02x\n", val);

    fts_i2c_read_r_reg(client, FTS_REG_CHARGER_MODE_EN, &val);
    count += snprintf(buf + count, PAGE_SIZE, "charge stat:0x%02x\n", val);

    fts_i2c_read_r_reg(client, FTS_REG_INT_CNT, &val);
    count += snprintf(buf + count, PAGE_SIZE, "INT count:0x%02x\n", val);

    fts_i2c_read_r_reg(client, FTS_REG_FLOW_WORK_CNT, &val);
...
/*  upgrade from fw bin file   example:echo "*.bin" > fts_upgrade_bin_r */
static DEVICE_ATTR(fts_upgrade_bin_r, S_IRUGO | S_IWUSR, fts_fwupgrade_rbin_show, fts_fwupgrade_rbin_store);
...
static struct attribute *fts_attributes[] = {
    ...
    &dev_attr_fts_upgrade_bin_r.attr,
