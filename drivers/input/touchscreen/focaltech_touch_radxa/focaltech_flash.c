...
struct upgrade_fw fw_list_r[] = {
    {FTS_VENDOR_ID, fw_file_r, sizeof(fw_file_r)},
    {FTS_VENDOR_ID2, fw_file_r2, sizeof(fw_file_r2)},
    {FTS_VENDOR_ID3, fw_file_r3, sizeof(fw_file_r3)},
};

struct upgrade_func *upgrade_func_list_r[] = {
    &upgrade_func_ft8006m_r,
};

struct fts_upgrade *fwupgrade_r = NULL;

/*****************************************************************************
* Static function prototypes
*****************************************************************************/
u16 fts_crc16_calc_host_r(u8 *pbuf, u16 length)
{
    u16 ecc = 0;
    u16 i = 0;
    u16 j = 0;
...
{
    return fts_crc16_calc_host_r(pbuf, length);
}
    ...
    u32 ecc_length)
{
    ...
    ret = fts_i2c_write_r(client, cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
    ...
    for (i = 0; i < 100; i++) {
        msleep(1);
        ret = fts_i2c_read_r(client, cmd, 1, val, 1);
        if (ret < 0) {
    ...
    ret = fts_i2c_read_r(client, cmd, 1, val, 2);
    if (ret < 0) {
...
static int fts_pram_ecc_cal_xor(struct i2c_client *client)
{
    ...
    ret = fts_i2c_read_r_reg(client, FTS_ROMBOOT_CMD_ECC, &reg_val);
    if (ret < 0) {
        FTS_ERROR("read pramboot ecc fail");
        return ret;
    }
...
{
    if ((NULL == fwupgrade_r) || (NULL == fwupgrade_r->func)) {
        FTS_ERROR("fwupgrade_r/func is null");
        return -EINVAL;
    }

    if (fwupgrade_r->func->newmode) {
...
static int fts_pram_write_buf(struct i2c_client *client, u8 *buf, u32 len)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts_i2c_write_r(client, packet_buf, packet_len + FTS_CMD_WRITE_LEN);
        if (ret < 0) {
...
static int fts_pram_start(struct i2c_client *client)
{
    ...
    ret = fts_i2c_write_r(client, &cmd, 1);
    if (ret < 0) {
...
static int fts_pram_write_remap(struct i2c_client *client)
{
    ...
    u8 *pb_buf = NULL;
    u32 pb_len = 0;
    struct fts_upgrade *upg = fwupgrade_r;
...
static int fts_pram_init(struct i2c_client *client)
{
    ...
    ret = fts_i2c_read_r(client, wbuf, 1, &reg_val, 1);
    if (ret < 0) {
    ...
    wbuf[1] = reg_val;
    wbuf[2] = 0x00;
    ret = fts_i2c_write_r(client, wbuf, 3);
    if (ret < 0) {
...
static int fts_pram_write_init(struct i2c_client *client)
{
    ...
    fts_fwupg_get_boot_state_r(client, &status);
    if (status != FTS_RUN_IN_ROM) {
        ...
        ret = fts_fwupg_reset_to_romboot_r(client);
        if (ret < 0) {
    ...
    state = fts_fwupg_check_state_r(client, FTS_RUN_IN_PRAM);
    if (!state) {
...
/************************************************************************
* Name: fts_fwupg_check_fw_valid_r
* Brief: check fw in tp is valid or not
* Input:
* Output:
* Return: return true if fw is valid, otherwise return false
***********************************************************************/
bool fts_fwupg_check_fw_valid_r(struct i2c_client *client)
{
    int ret = 0;

    ret = fts_wait_tp_to_valid_r(client);
    if (ret < 0) {
...
/************************************************************************
* Name: fts_fwupg_get_boot_state_r
* Brief: read boot id(rom/pram/bootloader), confirm boot environment
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_fwupg_get_boot_state_r(struct i2c_client *client, enum FW_STATUS *fw_sts)
{
    int ret = 0;
    u8 cmd[4] = { 0 };
    u32 cmd_len = 0;
    u8 val[2] = { 0 };
    struct ft_chip_t ids = fts_data_r->ic_info.ids;
    struct fts_upgrade *upg = fwupgrade_r;
    ...
    if (upg->func->hid_supported)
        fts_i2c_hid2std_r(client);

    cmd[0] = FTS_CMD_START1;
    cmd[1] = FTS_CMD_START2;
    ret = fts_i2c_write_r(client, cmd, 2);
    if (ret < 0) {
    ...
    if (fts_data_r->ic_info.is_incell)
        cmd_len = FTS_CMD_READ_ID_LEN_INCELL;
    else
        cmd_len = FTS_CMD_READ_ID_LEN;
    ret = fts_i2c_read_r(client, cmd, cmd_len, val, 2);
    if (ret < 0) {
...
/************************************************************************
* Name: fts_fwupg_reset_in_boot_r
* Brief: RST CMD(07), reset to romboot(bootloader) in boot environment
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_fwupg_reset_in_boot_r(struct i2c_client *client)
{
    int ret = 0;
...
/************************************************************************
* Name: fts_fwupg_reset_to_boot_r
* Brief: reset to boot environment
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_fwupg_reset_to_boot_r(struct i2c_client *client)
{
    int ret = 0;

    FTS_INFO("send 0xAA and 0x55 to FW, reset to boot environment");

    ret = fts_i2c_write_r_reg(client, FTS_REG_UPGRADE, FTS_UPGRADE_AA);
    if (ret < 0) {
    ...
    ret = fts_i2c_write_r_reg(client, FTS_REG_UPGRADE, FTS_UPGRADE_55);
    if (ret < 0) {
...
/************************************************************************
* Name: fts_fwupg_reset_to_romboot_r
* Brief: reset to romboot, to load pramboot
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_fwupg_reset_to_romboot_r(struct i2c_client *client)
{
    int ret = 0;
    int i = 0;
    u8 cmd = FTS_CMD_RESET;
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        fts_fwupg_get_boot_state_r(client, &state);
...
/************************************************************************
* Name: fts_fwupg_enter_into_boot_r
* Brief: enter into boot environment, ready for upgrade
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_fwupg_enter_into_boot_r(struct i2c_client *client)
{
    int ret = 0;
    bool fwvalid = false;
    bool state = false;
    ...
    fwvalid = fts_fwupg_check_fw_valid_r(client);
    if (fwvalid) {
        ret = fts_fwupg_reset_to_boot_r(client);
        if (ret < 0) {
            FTS_ERROR("enter into romboot/bootloader fail");
            return ret;
        }
    } else if (upg->func->read_boot_id_need_reset) {
        ret = fts_fwupg_reset_in_boot_r(client);
        if (ret < 0) {
    ...
    if (upg->func->pramboot_supported) {
        ...
        /* bootloader */
        state = fts_fwupg_check_state_r(client, FTS_RUN_IN_BOOTLOADER);
        if (!state) {
    ...
    int retries_delay)
{
    ...
    for (i = 0; i < retries; i++) {
        ...
        fts_i2c_read_r(client, &cmd , 1, val, FTS_CMD_FLASH_STATUS_LEN);
...
/************************************************************************
 * Name: fts_fwupg_erase_r
 * Brief: erase flash area
 * Input: delay - delay after erase
 * Output:
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_fwupg_erase_r(struct i2c_client *client, u32 delay)
{
    int ret = 0;
    u8 cmd = 0;
    bool flag = false;
 ...
 ***********************************************************************/
int fts_fwupg_ecc_cal_r(struct i2c_client *client, u32 saddr, u32 len)
{
    int ret = 0;
    u32 i = 0;
    ...
    ret = fts_i2c_write_r(client, wbuf, 1);
    if (ret < 0) {
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = fts_i2c_write_r(client, wbuf, FTS_CMD_ECC_CAL_LEN);
        if (ret < 0) {
    ...
    ret = fts_i2c_read_r(client, wbuf, 1, val, ecc_len);
    if (ret < 0) {
...
static int fts_flash_read_buf(struct i2c_client *client, u32 saddr, u8 *buf, u32 len)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts_i2c_write_r(client, wbuf, FTS_CMD_READ_LEN);
        if (ret < 0) {
        ...
        ret = fts_i2c_read_r(client, NULL, 0, buf + offset, packet_len);
        if (ret < 0) {
...
static int fts_flash_read(struct i2c_client *client, u32 addr, u8 *buf, u32 len)
{
    ...
    ret = fts_fwupg_enter_into_boot_r(client);
    if (ret < 0) {
...
/************************************************************************
* Name: fts_upgrade_bin_r
* Brief:
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_upgrade_bin_r(struct i2c_client *client, char *fw_name, bool force)
{
    int ret = 0;
    u32 fw_file_r_len = 0;
    u8 *fw_file_r_buf = NULL;
    struct fts_upgrade *upg = fwupgrade_r;
    ...
    ret = fts_read_file(fw_name, &fw_file_r_buf);
    ...
    fw_file_r_len = ret;
    FTS_INFO("fw bin file len:%d", fw_file_r_len);
    if (force) {
        if (upg->func->force_upgrade) {
            ret = upg->func->force_upgrade(client, fw_file_r_buf, fw_file_r_len);
        } else {
        ...
        if (upg->func->lic_upgrade) {
            ret = upg->func->lic_upgrade(client, fw_file_r_buf, fw_file_r_len);
        } else {
            FTS_INFO("lic_upgrade function is null, no upgrade");
        }
#endif
        if (upg->func->upgrade) {
            ret = upg->func->upgrade(client, fw_file_r_buf, fw_file_r_len);
        } else {
    ...
    if (ret < 0) {
        ...
        fts_fwupg_reset_in_boot_r(client);
        goto err_bin;
    }
...
static int fts_lic_get_vid_in_tp(struct i2c_client *client, u16 *vid)
{
    ...
    ret = fts_i2c_read_r_reg(client, FTS_REG_VENDOR_ID, &val[0]);
    if (fts_data_r->ic_info.is_incell)
        ret = fts_i2c_read_r_reg(client, FTS_REG_MODULE_ID, &val[1]);
    if (ret < 0) {
...
static int fts_lic_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
    ...
    ret = fts_i2c_read_r_reg(client, FTS_REG_LIC_VER, ver);
    if (ret < 0) {
...
static int fts_param_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
    ...
    ret = fts_i2c_read_r_reg(client, FTS_REG_IDE_PARA_VER_ID, ver);
    if (ret < 0) {
...
static int fts_param_get_ver_in_host(u8 *ver)
{
    ...
    if ((!upg) || (!upg->func) || (!upg->fw) || (!ver)) {
        FTS_ERROR("fts_data_r/upgrade/func/fw/ver is NULL");
        return -EINVAL;
    }
...
static int fts_param_ide_in_host(void)
{
    ...
    if ((!upg) || (!upg->func) || (!upg->fw)) {
        FTS_ERROR("fts_data_r/upgrade/func/fw is NULL");
        return -EINVAL;
    }
...
static int fts_param_ide_in_tp(struct i2c_client *client, u8 *val)
{
    ...
    ret = fts_i2c_read_r_reg(client, FTS_REG_IDE_PARA_STATUS, val);
    if (ret < 0) {
...
static int fts_fwupg_get_ver_in_tp(struct i2c_client *client, u8 *ver)
{
    ...
    ret = fts_i2c_read_r_reg(client, FTS_REG_FW_VER, ver);
    if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_fwupg_upgrade_r(struct i2c_client *client, struct fts_upgrade *upg)
{
    int ret = 0;
...
void fts_fwupg_auto_upgrade(struct fts_ts_data *ts_data)
{
    ...
    ret = fts_fwupg_upgrade_r(client, upg);
    if (ret < 0)
...
int fts_fwupg_get_vendorid(struct fts_ts_data *ts_data, u16 *vid)
{
    ...
    fwvalid = fts_fwupg_check_fw_valid_r(client);
    if (fwvalid) {
        ret = fts_i2c_read_r_reg(client, FTS_REG_VENDOR_ID, &vendor_id);
        if (ts_data->ic_info.is_incell)
            ret = fts_i2c_read_r_reg(client, FTS_REG_MODULE_ID, &module_id);
    } else {
...
/*
 * fts_fwupg_get_fw_file_r - get upgrade fw file in host driver
 *
 * return 0 if success, otherwise return error code
 *
 ...
 */
static int fts_fwupg_get_fw_file_r(struct fts_ts_data *ts_data)
{
    struct upgrade_fw *fw = &fw_list_r[0];
    struct fts_upgrade *upg = fwupgrade_r;
...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    ret = fts_fwupg_get_fw_file_r(ts_data);
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
    ...
    struct upgrade_func *func = upgrade_func_list_r[0];
    int func_count = sizeof(upgrade_func_list_r) / sizeof(upgrade_func_list_r[0]);
    ...
    fwupgrade_r = (struct fts_upgrade *)kzalloc(sizeof(*fwupgrade_r), GFP_KERNEL);
    if (NULL == fwupgrade_r) {
    ...
    if (1 == func_count) {
        fwupgrade_r->func = func;
    } else {
        for (i = 0; i < func_count; i++) {
            func = upgrade_func_list_r[i];
