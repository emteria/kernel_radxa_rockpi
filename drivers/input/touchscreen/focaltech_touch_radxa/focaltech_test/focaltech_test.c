...
#if 1
    if (NULL == fts_data_r) {
        FTS_TEST_ERROR("fts_data_r is null, no test");
        return -EINVAL;
    }
    ret = fts_i2c_read_r(fts_data_r->client, writebuf, writelen, readbuf, readlen);
#else
    ret = fts_i2c_read_r(writebuf, writelen, readbuf, readlen);
#endif
...
static int fts_test_i2c_write(u8 *writebuf, int writelen)
{
    ...
    ret = fts_i2c_write_r(fts_data_r->client, writebuf, writelen);
#else
    ret = fts_i2c_write_r(writebuf, writelen);
#endif
...
static int fts_test_func_init(void)
{
    int i = 0;
    int j = 0;
    int ic_stype = fts_data_r->ic_info.ids.type;
