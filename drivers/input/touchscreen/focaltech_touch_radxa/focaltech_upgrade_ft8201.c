...
static int read_3gamma(struct i2c_client *client, u8 **gamma, u16 *len)
{
    ...
    cmd[2] = (u8)(addr >> 8);
    cmd[3] = (u8)addr;
    fts_i2c_write_r(client, cmd, 4);
    msleep(10);
    ret = fts_i2c_read_r(client, NULL, 0, gamma_header, 0x20);
    if (ret < 0) {
    ...
    *gamma = kzalloc(MAX_GAMMA_LEN, GFP_KERNEL);
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        fts_i2c_write_r(client, cmd, 4);
        msleep(10);
        ret = fts_i2c_read_r(client, NULL, 0, pgamma + i * 256, packet_len);
        if (ret < 0) {
...
static bool fts_ft8006m_check_ide(u8 *buf, u32 len)
{
    ...
    off = upgrade_func_ft8006m_r.paramcfgoff;
    ...
    u32 len)
{
    ...
    ecc_in_host = fts_flash_write_buf_r(client, saddr, buf, len, 1);
    if (ecc_in_host < 0 ) {
    ...
    /* ecc */
    ecc_in_tp = fts_fwupg_ecc_cal_r(client, saddr, len);
    if (ecc_in_tp < 0 ) {
...
static int fts_ft8006m_param_flash(struct i2c_client *client, u8 *buf, u32 len)
{
    ...
    ret = fts_fwupg_erase_r(client, delay);
    if (ret < 0) {
    ...
    /* write flash */
    start_addr = upgrade_func_ft8006m_r.paramcfgoff;
    ...
    start_addr = upgrade_func_ft8006m_r.paramcfg2off;
    paramcfg_len = FTS_MAX_LEN_SECTOR;
    tmpbuf = buf + start_addr;
    ret = fts_ft8006m_write_ecc(client, start_addr, tmpbuf, paramcfg_len);
    if (ret < 0 ) {
...
static int fts_ft8006m_upgrade(struct i2c_client *client, u8 *buf, u32 len)
{
    ...
    off = upgrade_func_ft8006m_r.appoff + FTS_APPINFO_OFF + FTS_APPINFO_APPLEN_OFF;
    app_1_len = (((u32)buf[off] << 8) + buf[off + 1]);
    off = upgrade_func_ft8006m_r.appoff + FTS_APPINFO_OFF + FTS_APPINFO_APPLEN2_OFF;
    ...
    /* write flash */
    start_addr = upgrade_func_ft8006m_r.appoff;
    tmpbuf = buf + start_addr;
...
static int fts_ft8006m_lic_upgrade(struct i2c_client *client, u8 *buf, u32 len)
{
    ...
    ret = fts_i2c_read_r_reg(client, LIC_REG_2, &val);
    ...
    start_addr = upgrade_func_ft8006m_r.licoff;
...
struct upgrade_func upgrade_func_ft8006m_r = {
    .ctype = {0x07, 0x10},
    .fwveroff = 0x510E,
    .fwcfgoff = 0x0F80,
    .appoff = 0x5000,
    ...
    .pramboot = pb_file_ft8006m_r,
    .pb_length = sizeof(pb_file_ft8006m_r),
    .hid_supported = false,
