...
static u8 gtp_change_x2y2 = TRUE;
static u8 gtp_x_reverse2 = FALSE;
static u8 gtp_y_reverse2 = TRUE;

static const char *goodix_ts_name = "goodix-ts1";
static struct workqueue_struct *goodix_wq2;
struct i2c_client * i2c_connect_client2 = NULL; 
u8 config2[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
...
void gtp_reset_guitar2(struct i2c_client *client, s32 ms);
s32 gtp_send_cfg2(struct i2c_client *client);
void gtp_int_sync2(s32 ms, struct goodix_ts_data *ts);
...
void gtp_recovery_reset2(struct i2c_client *client);
static s32 gtp_esd_recovery(struct i2c_client *client);
s32 gtp_fw_startup2(struct i2c_client *client);
...
*********************************************************/
s32 gtp_i2c_read2(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    if((retries >= 5))
    {
        ...
        { 
            gtp_recovery_reset2(client);
        }
        else
    #endif
        {
            gtp_reset_guitar2(client, 10);  
        }
    }
    return ret;
}
...
*********************************************************/
s32 gtp_i2c_write2(struct i2c_client *client,u8 *buf,s32 len)
{
    struct i2c_msg msg;
    s32 ret = -1;
    s32 retries = 0;
...
*********************************************************/
s32 gtp_i2c_read_dbl_check2(struct i2c_client *client, u16 addr, u8 *rxbuf, int len)
{
    u8 buf[16] = {0};
    u8 confirm_buf[16] = {0};
    u8 retry = 0;
    ...
    while (retry++ < 3)
    {
        ...
        buf[0] = (u8)(addr >> 8);
        buf[1] = (u8)(addr & 0xFF);
        gtp_i2c_read2(client, buf, len + 2);
        
        memset(confirm_buf, 0xAB, 16);
        confirm_buf[0] = (u8)(addr >> 8);
        confirm_buf[1] = (u8)(addr & 0xFF);
        gtp_i2c_read2(client, confirm_buf, len + 2);
...
s32 gtp_send_cfg2(struct i2c_client *client)
{
    ...
    if (ts->fixed_cfg)
    {
        GTP_INFO("Ic fixed config2, no config2 sent!");
        return 0;
    }
    else if (ts->pnl_init_error)
    {
        GTP_INFO("Error occured in init_panel, no config2 sent");
        return 0;
    }
    
    GTP_INFO("Driver send config2.");
    for (retry = 0; retry < 5; retry++)
    {
        ret = gtp_i2c_write2(client, config2 , GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
        if (ret > 0)
        {
            break;
        }
...
Output:
    None.
*********************************************************/
void gtp_irq_disable2(struct goodix_ts_data *ts)
{
...
Output:
    None.
*********************************************************/
void gtp_irq_enable2(struct goodix_ts_data *ts)
{
...
{
    struct goodix_ts_data *ts = i2c_get_clientdata(i2c_connect_client2);
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    if (DOZE_ENABLED == doze_status)
    {               
        ret = gtp_i2c_read2(i2c_connect_client2, doze_buf, 3);
        ...
        {     
                ...
                )
            {
                ...
                // clear 0x814B
                doze_buf[2] = 0x00;
                gtp_i2c_write2(i2c_connect_client2, doze_buf, 3);
			}
    ...
    ret = gtp_i2c_read2(ts->client, point_data, 12);
    if (ret < 0)
    {
    ...
    {
        ret = gtp_i2c_read2(ts->client, rqst_buf, 3);
        if (ret < 0)
        {
        ...
        case GTP_RQST_CONFIG:
            GTP_INFO("Request for config2.");
            ret = gtp_send_cfg2(ts->client);
            if (ret < 0)
            {
                GTP_ERROR("Request for config2 unresponded!");
            }
            else
            {
                rqst_buf[2] = GTP_RQST_RESPONDED;
                gtp_i2c_write2(ts->client, rqst_buf, 3);
                GTP_INFO("Request for config2 responded!");
            }
            break;
    ...
    if (touch_num > 1)
    {
        ...
        ret = gtp_i2c_read2(ts->client, buf, 2 + 8 * (touch_num - 1)); 
...
exit_work_func:
    ...
    {
        ret = gtp_i2c_write2(ts->client, end_cmd, 3);
        if (ret < 0)
        {
...
static s8 gtp_enter_doze(struct goodix_ts_data *ts)
{
    ...
    while(retry++ < 5)
    {
        ...
        ret = gtp_i2c_write2(ts->client, i2c_control_buf, 3);
        if (ret < 0)
        {
...
static s8 gtp_enter_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        ret = gtp_i2c_read2(ts->client, status_buf, 3);
        if (ret < 0)
        {
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        for (retry = 0; retry < 10; ++retry)
        {
            // hold ss51 & dsp
            opr_buf[2] = 0x0C;
            ret = gtp_i2c_write2(ts->client, opr_buf, 3);
            if (FAIL == ret)
            {
            ...
            opr_buf[2] = 0x00;
            ret = gtp_i2c_read2(ts->client, opr_buf, 3);
            if (FAIL == ret)
            {
    ...
    while(retry++ < 5)
    {
        gtp_reset_guitar2(ts->client, 20);
    ...
    while(retry++ < 10)
    {
        ...
        gtp_irq_disable2(ts);
        gtp_reset_guitar2(ts->client, 10);
        gtp_irq_enable2(ts);
...
static s32 gtp_get_info(struct goodix_ts_data *ts)
{
    ...
    ret = gtp_i2c_read2(ts->client, opr_buf, 6);
    if (ret < 0)
    {
        return FAIL;
    }
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    u8 *send_cfg_buf[] = {gtp_dat_10_12, cfg_info_group2, cfg_info_group3,
                            cfg_info_group4, cfg_info_group5, cfg_info_group6};
    u8 cfg_info_len[] = { CFG_GROUP_LEN(gtp_dat_10_12),
    ...
    if(m89or1012){
	    if (ts->cfg_file_num) {
		    send_cfg_buf[0] = gtp_dat_8_9_12;
		    cfg_info_len[0] =  CFG_GROUP_LEN(gtp_dat_8_9_12);
	    } else {
		    send_cfg_buf[0] = gtp_dat_8_92;
		    cfg_info_len[0] =  CFG_GROUP_LEN(gtp_dat_8_92);
	    }
    }
    
    if (bgt911) {
    	send_cfg_buf[0] = gtp_dat_gt112;
		cfg_info_len[0] =  CFG_GROUP_LEN(gtp_dat_gt112);
    }

    if (bgt9110) {
	    send_cfg_buf[0] = gtp_dat_gt91102;
	    cfg_info_len[0] =  CFG_GROUP_LEN(gtp_dat_gt91102);
    }

	if (bgt9111) {
		send_cfg_buf[0] = gtp_dat_gt91112;
		cfg_info_len[0] =  CFG_GROUP_LEN(gtp_dat_gt91112);
	}

	if (bgt970) {
		send_cfg_buf[0] = gtp_dat_9_72;
		cfg_info_len[0] = CFG_GROUP_LEN(gtp_dat_9_72);
	}

	if (bgt910) {
		send_cfg_buf[0] = gtp_dat_72;
		cfg_info_len[0] = CFG_GROUP_LEN(gtp_dat_72);
	}

    GTP_DEBUG_FUNC();
    GTP_DEBUG("config2 Groups\' Lengths: %d, %d, %d, %d, %d, %d", 
...
#endif
    {
        ret = gtp_i2c_read_dbl_check2(ts->client, 0x41E4, opr_buf, 1);
        if (SUCCESS == ret) 
        {
            if (opr_buf[0] != 0xBE)
            {
                ts->fw_error = 1;
                GTP_ERROR("Firmware error, no config2 sent!");
                return -1;
            }
        }
    }
    ...
    else
    {
        ...
        msleep(50);
    #endif
        ret = gtp_i2c_read_dbl_check2(ts->client, GTP_REG_SENSOR_ID, &sensor_id, 1);
        if (SUCCESS == ret)
        {
            if (sensor_id >= 0x06)
            {
                GTP_ERROR("Invalid sensor_id(0x%02X), No config2 Sent!", sensor_id);
                ts->pnl_init_error = 1;
                return -1;
            }
        }
        ...
        {
            GTP_ERROR("Failed to get sensor_id, No config2 sent!");
            ts->pnl_init_error = 1;
            return -1;
        }
    ...
    GTP_INFO("CTP_CONFIG_GROUP%d used, config2 length: %d", sensor_id + 1, ts->gtp_cfg_len);
    
    if (ts->gtp_cfg_len < GTP_CONFIG_MIN_LENGTH)
    {
        GTP_ERROR("config2 Group%d is INVALID config2 GROUP(Len: %d)! NO config2 Sent! You need to check you header file CFG_GROUP section!", sensor_id+1, ts->gtp_cfg_len);
        ts->pnl_init_error = 1;
        return -1;
    }
...
#endif
    {
        ret = gtp_i2c_read_dbl_check2(ts->client, GTP_REG_CONFIG_DATA, &opr_buf[0], 1);
        
        if (ret == SUCCESS)
        {
            GTP_DEBUG("CFG_GROUP%d config2 Version: %d, 0x%02X; IC config2 Version: %d, 0x%02X", sensor_id+1, 
                        send_cfg_buf[sensor_id][0], send_cfg_buf[sensor_id][0], opr_buf[0], opr_buf[0]);
            
            if (opr_buf[0] < 90)    
            {
                GTP_INFO("  <%s>_%d \n", __func__, __LINE__);
                grp_cfg_version2 = send_cfg_buf[sensor_id][0];       // backup group config2 version
                send_cfg_buf[sensor_id][0] = 0x00;
                ts->fixed_cfg = 0;
            }
            else        // treated as fixed config2, not send config2
            {
                GTP_INFO("Ic fixed config2 with config2 version(%d, 0x%02X)", opr_buf[0], opr_buf[0]);
                ts->fixed_cfg = 1;
                gtp_get_info(ts);
                return 0;
            }
        ...
        {
            GTP_ERROR("Failed to get ic config2 version!No config2 sent!");
            return -1;
        }
    }
    
    memset(&config2[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
    memcpy(&config2[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id], ts->gtp_cfg_len);

#if GTP_CUSTOM_CFG
    config2[RESOLUTION_LOC]     = (u8)GTP_MAX_WIDTH;
    config2[RESOLUTION_LOC + 1] = (u8)(GTP_MAX_WIDTH>>8);
    config2[RESOLUTION_LOC + 2] = (u8)GTP_MAX_HEIGHT;
    config2[RESOLUTION_LOC + 3] = (u8)(GTP_MAX_HEIGHT>>8);
    
    if (GTP_INT_TRIGGER == 0)  //RISING
    {
        config2[TRIGGER_LOC] &= 0xfe; 
    }
    else if (GTP_INT_TRIGGER == 1)  //FALLING
    {
        config2[TRIGGER_LOC] |= 0x01;
    }
    ...
    config2[ts->gtp_cfg_len] = (~check_sum) + 1;

#else // driver not send config2

    ts->gtp_cfg_len = GTP_CONFIG_MAX_LENGTH;
    ret = gtp_i2c_read2(ts->client, config2, ts->gtp_cfg_len + GTP_ADDR_LENGTH);
    if (ret < 0)
    {
        GTP_ERROR("Read config2 Failed, Using Default Resolution & INT Trigger!");
        //ts->abs_x_max = GTP_MAX_WIDTH;
        //ts->abs_y_max = GTP_MAX_HEIGHT;
        ts->int_trigger_type = GTP_INT_TRIGGER;
    }
    ...
    {
        ts->abs_x_max = (config2[RESOLUTION_LOC + 1] << 8) + config2[RESOLUTION_LOC];
        ts->abs_y_max = (config2[RESOLUTION_LOC + 3] << 8) + config2[RESOLUTION_LOC + 2];
        ts->int_trigger_type = (config2[TRIGGER_LOC]) & 0x03; 
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        have_key = (config2[GTP_REG_HAVE_KEY - GTP_REG_CONFIG_DATA + 2] & 0x01);
        
        if (1 == ts->is_950)
        {
            driver_num = config2[GTP_REG_MATRIX_DRVNUM - GTP_REG_CONFIG_DATA + 2];
            sensor_num = config2[GTP_REG_MATRIX_SENNUM - GTP_REG_CONFIG_DATA + 2];
            if (have_key)
            {
                driver_num--;
            }
        ...
        {
            driver_num = (config2[CFG_LOC_DRVA_NUM] & 0x1F) + (config2[CFG_LOC_DRVB_NUM]&0x1F);
            if (have_key)
            {
                driver_num--;
            }
            sensor_num = (config2[CFG_LOC_SENS_NUM] & 0x0F) + ((config2[CFG_LOC_SENS_NUM] >> 4) & 0x0F);
...
#endif
    {
        ...
        if (ret < 0)
        {
            GTP_ERROR("Send config2 error.");
        }
        // set config2 version to CTP_CFG_GROUP, for resume to send config2
        config2[GTP_ADDR_LENGTH] = grp_cfg_version2;
        check_sum = 0;
        for (i = GTP_ADDR_LENGTH; i < ts->gtp_cfg_len; i++)
        {
            check_sum += config2[i];
...
static ssize_t gt91xx_config_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    ptr += sprintf(ptr, "==== GT9XX config2 init value====\n");

    for (i = 0 ; i < GTP_CONFIG_MAX_LENGTH ; i++)
    {
        ptr += sprintf(ptr, "0x%02X ", config2[i + 2]);
    ...
    ptr += sprintf(ptr, "==== GT9XX config2 real value====\n");
    gtp_i2c_read2(i2c_connect_client2, temp_data, GTP_CONFIG_MAX_LENGTH + 2);
    for (i = 0 ; i < GTP_CONFIG_MAX_LENGTH ; i++)
    {
...
static ssize_t gt91xx_config_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    ...
    if (copy_from_user(&config2[2], buffer, count))
    {
        GTP_ERROR("copy from user fail\n");
        return -EFAULT;
    }

    ret = gtp_send_cfg2(i2c_connect_client2);

    if (ret < 0)
    {
        GTP_ERROR("send config2 failed.");
    }
...
Output:
    read operation return.
        2: succeed, otherwise: failed
*******************************************************/
s32 gtp_read_version2(struct i2c_client *client, u16* version)
{
    s32 ret = -1;
    ...
    ret = gtp_i2c_read2(client, buf, sizeof(buf));
    if (ret < 0)
    {
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
    ...
    while(retry++ < 5)
    {
        ret = gtp_i2c_read2(client, test, 3);
        if (ret > 0)
        {
            return ret;
        }
...
s32 gtp_gt9xxf_init2(struct i2c_client *client)
{
    s32 ret = 0;
...
void gtp_get_chip_type2(struct goodix_ts_data *ts)
{
    u8 opr_buf[10] = {0x00};
    s32 ret = 0;
    ...
    ret = gtp_i2c_read_dbl_check2(ts->client, GTP_REG_CHIP_TYPE, opr_buf, 10);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (val == 89) {
		m89or101 = TRUE;
		gtp_change_x2y2 = FALSE;
		gtp_x_reverse2 = FALSE;
		gtp_y_reverse2 = FALSE;
	} else if (val == 101) {
		m89or101 = FALSE;
		gtp_change_x2y2 = TRUE;
		gtp_x_reverse2 = TRUE;
		gtp_y_reverse2 = FALSE;
	} else if (val == 911) {
		m89or101 = FALSE;
...
/*
    ...
    else//DPT
    {
		...
		gtp_x_reverse2 = TRUE;//FALSE;
		gtp_y_reverse2 = TRUE;
    }
 */
#if GTP_COMPATIBLE_MODE
    gtp_get_chip_type2(ts);
    
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ret = gtp_gt9xxf_init2(ts->client);
        if (FAIL == ret)
        {
    ...
    ret = gtp_read_version2(client, &version_info);
    if (ret < 0)
    {
...
static void gtp_esd_check_func(struct work_struct *work)
{
    ...
    ts = i2c_get_clientdata(i2c_connect_client2);
    ...
    if (i >= 3)
    {
    ...
    #endif
        {
            ...
            gtp_i2c_write_no_rst(ts->client, esd_buf, 5);
            msleep(50);
            gtp_reset_guitar2(ts->client, 50);
            msleep(50);
            gtp_send_cfg2(ts->client);
        }
    }
...
static struct of_device_id goodix_ts_dt_ids[] = {
    { .compatible = "goodix,gt9xx2" },
    { }
};

static struct i2c_driver goodix_ts_driver2 = {
...
Output:
    Executive Outcomes. 0---succeed.
********************************************************/
static int goodix_ts_init2(void)
{
    s32 ret;

    GTP_DEBUG_FUNC();   
    GTP_INFO("GTP driver installing...");
    goodix_wq2 = create_singlethread_workqueue("goodix_wq2");
    if (!goodix_wq2)
    {
...
#endif
    ret = i2c_add_driver(&goodix_ts_driver2);
    return ret; 
}
...
Output:
    Executive Outcomes. 0---succeed.
********************************************************/
static void goodix_ts_exit2(void)
{
    GTP_DEBUG_FUNC();
    GTP_INFO("GTP driver exited.");
    i2c_del_driver(&goodix_ts_driver2);
    if (goodix_wq2)
    {
        destroy_workqueue(goodix_wq2);
    }
}
//late_initcall(goodix_ts_init);
module_init(goodix_ts_init2);

module_exit(goodix_ts_exit2);
