...
st_update_msg update_msg2;
u16 show_len2;
u16 total_len2;
u8 got_file_flag2 = 0;  
u8 searching_file2 = 0;
...
s32 gup_fw_download_proc2(void *dir, u8 dwn_mode);
#endif
/*******************************************************
Function:
...
*********************************************************/
s32 gup_i2c_read2(struct i2c_client *client, u8 *buf, s32 len)
{
...
*********************************************************/
s32 gup_i2c_write2(struct i2c_client *client,u8 *buf,s32 len)
{
    struct i2c_msg msg;
    s32 ret=-1;
    s32 retries = 0;
...
static s32 gup_init_panel2(struct goodix_ts_data *ts)
{
    s32 ret = 0;
    s32 i = 0;
    u8 check_sum = 0;
...
static u8 gup_get_ic_msg(struct i2c_client *client, u16 addr, u8* msg, s32 len)
{
    ...
    for (i = 0; i < 5; i++)
    {
        if (gup_i2c_read2(client, msg, GTP_ADDR_LENGTH + len) > 0)
        {
            break;
        }
    }
...
static u8 gup_set_ic_msg(struct i2c_client *client, u16 addr, u8 val)
{
    ...
    for (i = 0; i < 5; i++)
    {
        if (gup_i2c_write2(client, msg, GTP_ADDR_LENGTH + 1) > 0)
        {
            break;
        }
    }
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    for(i=0; i<4; i++)
    {
        update_msg2.ic_fw_msg.hw_info[i] = buf[GTP_ADDR_LENGTH + 3 - i];
    } 
    GTP_DEBUG("IC Hardware info:%02x%02x%02x%02x", update_msg2.ic_fw_msg.hw_info[0], update_msg2.ic_fw_msg.hw_info[1],
                                                   update_msg2.ic_fw_msg.hw_info[2], update_msg2.ic_fw_msg.hw_info[3]);
    ...
    for(retry=0; retry<2; retry++)
    {
        ...
        update_msg2.force_update = buf[GTP_ADDR_LENGTH];
        if((0xBE != update_msg2.force_update)&&(!retry))
        {
    ...
    GTP_DEBUG("IC force update flag:0x%x", update_msg2.force_update);
    ...
    memset(update_msg2.ic_fw_msg.pid, 0, sizeof(update_msg2.ic_fw_msg.pid));
    memcpy(update_msg2.ic_fw_msg.pid, &buf[GTP_ADDR_LENGTH], 4);
    GTP_DEBUG("IC Product id:%s", update_msg2.ic_fw_msg.pid);
    ...
    /*|-----FLASH-----RAM-----|
      ...
      |------9110-----9110----|
      |------9110P----9111----|*/
    if(update_msg2.ic_fw_msg.pid[0] != 0)
    {
        if(!memcmp(update_msg2.ic_fw_msg.pid, "9111", 4))
        {
            GTP_DEBUG("IC Mapping Product id:%s", update_msg2.ic_fw_msg.pid);
            memcpy(update_msg2.ic_fw_msg.pid, "9110P", 5);
        }
    }
    
    update_msg2.ic_fw_msg.vid = buf[GTP_ADDR_LENGTH+4] + (buf[GTP_ADDR_LENGTH+5]<<8);
    GTP_DEBUG("IC version id:%04x", update_msg2.ic_fw_msg.vid);
    
    return SUCCESS;
}

s32 gup_enter_update_mode2(struct i2c_client *client)
{
    s32 ret = -1;
    s32 retry = 0;
    u8 rd_buf[3];
...
void gup_leave_update_mode2(struct goodix_ts_data *ts)
{
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    GTP_INFO("IC HARDWARE INFO:%02x%02x%02x%02x", update_msg2.ic_fw_msg.hw_info[0], update_msg2.ic_fw_msg.hw_info[1],
             update_msg2.ic_fw_msg.hw_info[2], update_msg2.ic_fw_msg.hw_info[3]);
    GTP_INFO("IC PID:%s", update_msg2.ic_fw_msg.pid);
    GTP_INFO("IC VID:%04x", update_msg2.ic_fw_msg.vid);

    if (!memcmp(fw_head->pid, "9158", 4) && !memcmp(update_msg2.ic_fw_msg.pid, "915S", 4))
    {
        GTP_INFO("Update GT915S to GT9158 directly!");
        return SUCCESS;
    }
    //First two conditions
    if (!memcmp(fw_head->hw_info, update_msg2.ic_fw_msg.hw_info, sizeof(update_msg2.ic_fw_msg.hw_info)))
    {
        fw_len = 42 * 1024;
    }
    else
    ...
    if (update_msg2.fw_total_len != fw_len)
    {
        GTP_ERROR("Inconsistent firmware size, Update aborted! Default size: %d(%dK), actual size: %d(%dK)", fw_len, fw_len/1024, update_msg2.fw_total_len, update_msg2.fw_total_len/1024);
        return FAIL;
    }
    GTP_INFO("Firmware length:%d(%dK)", update_msg2.fw_total_len, update_msg2.fw_total_len/1024);
    
    if (update_msg2.force_update != 0xBE)
    {
    ...
    // 20130523 start
    if (strlen(update_msg2.ic_fw_msg.pid) < 3)
    {
    ...
    else
    {
        for (i = 0; i < 3; i++)
        {
            if ((update_msg2.ic_fw_msg.pid[i] < 0x30) || (update_msg2.ic_fw_msg.pid[i] > 0x39))
            {
    ...
    if (pid_cmp_len < strlen(update_msg2.ic_fw_msg.pid))
    {
        pid_cmp_len = strlen(update_msg2.ic_fw_msg.pid);
    }
    
    if ((!memcmp(fw_head->pid, update_msg2.ic_fw_msg.pid, pid_cmp_len)) ||
            (!memcmp(update_msg2.ic_fw_msg.pid, "91XX", 4))||
            (!memcmp(fw_head->pid, "91XX", 4)))
    {
        ...
        //The third condition
        if (fw_head->vid > update_msg2.ic_fw_msg.vid)
        {
...
static s8 gup_update_config(struct i2c_client *client)
{
    ...
    if(NULL == update_msg2.cfg_file)
    {
        GTP_ERROR("[update_cfg]No need to upgrade config!");
        return FAIL;
    }
    file_len = update_msg2.cfg_file->f_op->llseek(update_msg2.cfg_file, 0, SEEK_END);
    ...
    update_msg2.cfg_file->f_op->llseek(update_msg2.cfg_file, 0, SEEK_SET);
    
    GTP_DEBUG("[update_cfg]Read config from file.");
    ret = update_msg2.cfg_file->f_op->read(update_msg2.cfg_file, (char*)pre_buf, file_len, &update_msg2.cfg_file->f_pos);
    if(ret<0)
    {
    ...
    while(i++ < 5)
    {
        ret = gup_i2c_write2(client, file_config, file_cfg_len);
        if(ret > 0)
        {
...
static void gup_search_file(s32 search_type)
{
        ...
        if (search_type & AUTO_SEARCH_BIN)
        {
            ...
            if (IS_ERR(pfile))
            {
                ...
                if (!IS_ERR(pfile))
                {
                    ...
                    got_file_flag2 |= BIN_FILE_READY;
                    update_msg2.file = pfile;
                }
            }
            else
            {
            ...
            if (got_file_flag2 & BIN_FILE_READY)
            {
            #if GTP_AUTO_UPDATE_CFG
    ...
    #if GTP_AUTO_UPDATE_CFG
        if ( (search_type & AUTO_SEARCH_CFG) && !(got_file_flag2 & CFG_FILE_READY) )
        {
            ...
            if (IS_ERR(pfile))
            {
                ...
                if (!IS_ERR(pfile))
                {
                    ...
                    got_file_flag2 |= CFG_FILE_READY;
                    update_msg2.cfg_file = pfile;
                }
            }
            else
            {
            ...
            if (got_file_flag2 & CFG_FILE_READY)
            {
                searching_file2 = 0;
                return;
            }
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    if (path)
    {
        ...
        update_msg2.file = filp_open(path, O_RDONLY, 0);

        if (IS_ERR(update_msg2.file))
        {
    ...
    else
    {
        ...
        update_msg2.fw_total_len = sizeof(gtp_default_FW) - FW_HEAD_LENGTH;
        ...
        GTP_DEBUG("Firmware actual size: %d(%dK)", update_msg2.fw_total_len, update_msg2.fw_total_len/1024);
        memcpy(fw_head, &gtp_default_FW[0], FW_HEAD_LENGTH);
    
        //check firmware legality
        fw_checksum = 0;
        for(i=0; i< update_msg2.fw_total_len; i+=2)
        {
        ...
        got_file_flag2 = HEADER_FW_READY;
        return SUCCESS;
    #else
        ...
        if (got_file_flag2 & CFG_FILE_READY)
        {
            ...
            _CLOSE_FILE(update_msg2.cfg_file);
    ...
    update_msg2.old_fs = get_fs();
    set_fs(KERNEL_DS);

    update_msg2.file->f_op->llseek(update_msg2.file, 0, SEEK_SET);
    update_msg2.fw_total_len = update_msg2.file->f_op->llseek(update_msg2.file, 0, SEEK_END);
    if (update_msg2.fw_total_len < (FW_HEAD_LENGTH + FW_SECTION_LENGTH*4+FW_DSP_ISP_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH))
    {
        GTP_ERROR("INVALID bin file(size: %d), update aborted.", update_msg2.fw_total_len);
        return FAIL;
    }    
    
    update_msg2.fw_total_len -= FW_HEAD_LENGTH;

    GTP_DEBUG("Bin firmware actual size: %d(%dK)", update_msg2.fw_total_len, update_msg2.fw_total_len/1024);
    
    update_msg2.file->f_op->llseek(update_msg2.file, 0, SEEK_SET);
    ret = update_msg2.file->f_op->read(update_msg2.file, (char*)buf, FW_HEAD_LENGTH, &update_msg2.file->f_pos);
    if (ret < 0)
    {
    ...
    {
        u16 temp;
        ret = update_msg2.file->f_op->read(update_msg2.file, (char*)buf, 2, &update_msg2.file->f_pos);
        if (ret < 0)
        {
...
static u8 gup_burn_proc(struct i2c_client *client, u8 *burn_buf, u16 start_addr, u16 total_length)
{
    ...
    while(burn_length < total_length)
    {
        ...
        {
            ret = gup_i2c_write2(client, wr_buf, GTP_ADDR_LENGTH + frame_length);
            if(ret <= 0)
            {
                GTP_ERROR("Write frame data i2c error.");
                continue;
            }
            ret = gup_i2c_read2(client, rd_buf, GTP_ADDR_LENGTH + frame_length);
            if(ret <= 0)
            {
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
    ...
    if (got_file_flag2 == HEADER_FW_READY)
    {
        ...
        {
            memcpy(buf, &gtp_default_FW[update_msg2.fw_total_len + FW_HEAD_LENGTH - offset], length);
        }
        return SUCCESS;
    }
#endif
    {
        ...
        if ( (update_msg2.file == NULL) || IS_ERR(update_msg2.file))
        {
        ...
        if(SEEK_SET == set_or_end)
        {
            update_msg2.file->f_pos = FW_HEAD_LENGTH + offset;
        }
        else    //seek end
        {
            update_msg2.file->f_pos = update_msg2.fw_total_len + FW_HEAD_LENGTH - offset;
        }
    
        ret = update_msg2.file->f_op->read(update_msg2.file, (char *)buf, length, &update_msg2.file->f_pos);
...
static u8 gup_recall_check2(struct i2c_client *client, u8* chk_src, u16 start_rd_addr, u16 chk_length)
{
    u8  rd_buf[PACK_SIZE + GTP_ADDR_LENGTH];
    s32 ret = 0;
...
static u8 gup_burn_fw_section2(struct i2c_client *client, u8 *fw_section, u16 start_addr, u8 bank_cmd )
{
    s32 ret = 0;
    u8  rd_buf[5];
    ...
    ret = gup_recall_check2(client, fw_section, start_addr, FW_SECTION_LENGTH);
    if(FAIL == ret)
    {
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
    ...
    update_msg2.fw_burned_len += FW_DSP_ISP_LENGTH;
    GTP_DEBUG("[burn_dsp_isp]Burned length:%d", update_msg2.fw_burned_len);
    ret = SUCCESS;
...
//    }
    ...
    ret = gup_burn_fw_section2(client, fw_ss51, 0xC000, 0x01);
    if(FAIL == ret)
    {
    ...
    ret = gup_burn_fw_section2(client, fw_ss51, 0xE000, 0x02);
    if(FAIL == ret)
    {
    ...
    ret = gup_burn_fw_section2(client, fw_ss51, 0xC000, 0x13);
    if(FAIL == ret)
    {
    ...
    ret = gup_burn_fw_section2(client, fw_ss51, 0xE000, 0x14);
    if(FAIL == ret)
    {
    ...
    update_msg2.fw_burned_len += (FW_SECTION_LENGTH*4);
    GTP_DEBUG("[burn_fw_ss51]Burned length:%d", update_msg2.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
    ...
    ret = gup_recall_check2(client, fw_dsp, 0x9000, FW_DSP_LENGTH);
    if(FAIL == ret)
    {
    ...
    update_msg2.fw_burned_len += FW_DSP_LENGTH;
    GTP_DEBUG("[burn_fw_dsp]Burned length:%d", update_msg2.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
    ...
    ret = gup_recall_check2(client, fw_boot, 0x9000, FW_BOOT_LENGTH);
    if(FAIL == ret)
    {
    ...
    update_msg2.fw_burned_len += FW_BOOT_LENGTH;
    GTP_DEBUG("[burn_fw_boot]Burned length:%d", update_msg2.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
    ...
    if(update_msg2.fw_burned_len >= update_msg2.fw_total_len)
    {
    ...
    ret = gup_load_section_file(fw_boot_isp, (update_msg2.fw_burned_len - FW_DSP_ISP_LENGTH), FW_BOOT_ISP_LENGTH, SEEK_SET);
    if(FAIL == ret)
    {
    ...
    ret = gup_recall_check2(client, fw_boot_isp, 0x9000, FW_BOOT_ISP_LENGTH);
    if(FAIL == ret)
    {
    ...
    update_msg2.fw_burned_len += FW_BOOT_ISP_LENGTH;
    GTP_DEBUG("[burn_fw_boot_isp]Burned length:%d", update_msg2.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_link(struct i2c_client *client)
{
    ...
    offset = update_msg2.fw_burned_len - FW_DSP_ISP_LENGTH;
    ...
    update_msg2.fw_burned_len += FW_GLINK_LENGTH;
    GTP_DEBUG("[burn_fw_link]Burned length:%d", update_msg2.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_gwake_section(struct i2c_client *client, u8 *fw_section, u16 start_addr, u32 len, u8 bank_cmd )
{
    ...
    ret = gup_recall_check2(client, fw_section, start_addr, len);
    if(FAIL == ret)
    {
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
    ...
    start_index = update_msg2.fw_burned_len - FW_DSP_ISP_LENGTH;
    ...
    update_msg2.fw_burned_len += FW_GWAKE_LENGTH;
    GTP_DEBUG("[burn_fw_gwake]Burned length:%d", update_msg2.fw_burned_len);
    ret = SUCCESS;
...
s32 gup_update_proc2(void *dir)
{
    ...
    {
        u8 timeout = 0;
        searching_file2 = 0;     // exit .bin update file searching 
        GTP_INFO("Exiting searching .bin update file...");
        while ((show_len2 != 200) && (show_len2 != 100) && (timeout++ < 100))     // wait for auto update quitted completely
        {
            msleep(100);
        }
    }
    ...
    {
        return gup_fw_download_proc2(dir, GTP_FL_FW_BURN);
    }
#endif

    update_msg2.file = NULL;
...
#endif
    ret = gup_enter_update_mode2(i2c_connect_client);
    if(FAIL == ret)
    {
...
file_fail:
	if (update_msg2.file && !IS_ERR(update_msg2.file))
	{
        if (update_msg2.old_fs)
        {
            set_fs(update_msg2.old_fs);
        }
		filp_close(update_msg2.file, NULL);
	}
...
u8 gup_init_update_proc(struct goodix_ts_data *ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        thread = kthread_run(gup_update_proc2, "update", "fl update");
    }
    else
#endif
    {    
        thread = kthread_run(gup_update_proc2, (void*)NULL, "guitar_update");
    }
    if (IS_ERR(thread))
    {
...
u8 i2c_opr_buf2[GTP_ADDR_LENGTH + FL_PACK_SIZE] = {0};
u8 chk_cmp_buf2[FL_PACK_SIZE] = {0};
...
u8 gup_check_fs_mounted2(char *path_name)
{
...
s32 i2c_write_bytes2(struct i2c_client *client, u16 addr, u8 *buf, s32 len)
{
    s32 ret = 0;
    s32 write_bytes = 0;
    s32 retry = 0;
    ...
    while (len > 0)
    {
        i2c_opr_buf2[0] = (u8)(addr >> 8);
        i2c_opr_buf2[1] = (u8)(addr & 0xFF);
        if (len > FL_PACK_SIZE)
        {
        ...
        memcpy(i2c_opr_buf2 + 2, tx_buf, write_bytes);
        for (retry = 0; retry < 5; ++retry)
        {
            ret = gup_i2c_write2(client, i2c_opr_buf2, write_bytes + GTP_ADDR_LENGTH);
            if (ret == 1)
            {
                break;
            }
...
s32 i2c_read_bytes2(struct i2c_client *client, u16 addr, u8 *buf, s32 len)
{
    s32 ret = 0;
    s32 read_bytes = 0;
    s32 retry = 0;
    ...
    while (len > 0)
    {
        ...
        for (retry = 0; retry < 5; ++retry)
        {
            ret = gup_i2c_read2(client, i2c_opr_buf2, read_bytes + GTP_ADDR_LENGTH);
            if (ret == 2)
            {
                break;
            }
        ...
        memcpy(tx_buf, i2c_opr_buf2 + 2, read_bytes);
        addr += read_bytes;
        len -= read_bytes;
        tx_buf += read_bytes;
    }
...
{
    u8 buf;
    i2c_read_bytes2(i2c_connect_client, addr, &buf, 1);

    buf = (buf & (~((u8)1 << bit))) | ((u8)val << bit);

    i2c_write_bytes2(i2c_connect_client, addr, &buf, 1);
}
...
static void gup_clk_count_init(s32 bCh, s32 bCNT)
{
    ...
    buf = 0;
    i2c_write_bytes2(i2c_connect_client, _bRW_MISCTL__MEA_MODE, &buf, 1);
    //_bRW_MISCTL__MEA_SRCSEL = 8 + bCh; //From GIO1
    buf = 8 + bCh;
    i2c_write_bytes2(i2c_connect_client, _bRW_MISCTL__MEA_SRCSEL, &buf, 1);
    //_wRW_MISCTL__MEA_MAX_NUM = bCNT; //Set the Measure Counts = 1
    buf = bCNT;
    i2c_write_bytes2(i2c_connect_client, _wRW_MISCTL__MEA_MAX_NUM, &buf, 1);
...
static u32 gup_clk_count_get(void)
{
    ...
    {
        i2c_read_bytes2(i2c_connect_client, _bRO_MISCTL__MEA_RDY, buf, 1);
        ready = buf[0];
    }
    ...
    gup_bit_write(_fRW_MISCTL__MEA, 0, 0);
    i2c_read_bytes2(i2c_connect_client, _dRO_MISCTL__MEA_VAL, buf, 4);
...
u8 gup_clk_dac_setting2(int dac)
{
    s8 buf1, buf2;
    
    i2c_read_bytes2(i2c_connect_client, _wRW_MISCTL__RG_DMY, &buf1, 1);
    i2c_read_bytes2(i2c_connect_client, _bRW_MISCTL__RG_OSC_CALIB, &buf2, 1);

    buf1 = (buf1 & 0xFFCF) | ((dac & 0x03) << 4);
    buf2 = (dac >> 2) & 0x3f;

    i2c_write_bytes2(i2c_connect_client, _wRW_MISCTL__RG_DMY, &buf1, 1);
    i2c_write_bytes2(i2c_connect_client, _bRW_MISCTL__RG_OSC_CALIB, &buf2, 1);
...
static void gup_sys_clk_init(void)
{
    ...
    buf = 0;
    i2c_write_bytes2(i2c_connect_client, _bRW_MISCTL__RG_LDO_A18_PWD, &buf, 1);
    //_bRW_MISCTL__RG_BG_PWD = 0; //DrvMISCTL_BG_PowerON
    buf = 0;
    i2c_write_bytes2(i2c_connect_client, _bRW_MISCTL__RG_BG_PWD, &buf, 1);
    //_bRW_MISCTL__RG_CLKGEN_PWD = 0; //DrvMISCTL_CLKGEN_PowerON
    buf = 0;
    i2c_write_bytes2(i2c_connect_client, _bRW_MISCTL__RG_CLKGEN_PWD, &buf, 1);
    ...
    //gup_clk_dac_setting2(60);
    //_bRW_MISCTL__OSC_CK_SEL = 1;;
    buf = 1;
    i2c_write_bytes2(i2c_connect_client, _bRW_MISCTL__OSC_CK_SEL, &buf, 1);
}

s32 gup_clk_calibration2(void)
{
    u8 buf;
    //u8 trigger;
    s32 i;
    ...
    i2c_write_bytes2(i2c_connect_client, _rRW_MISCTL__SWRST_B0_, &buf, 1);
    ...
    for (i = INIT_CLK_DAC; i < MAX_CLK_DAC; i++)
    {
        ...
        gup_clk_dac_setting2(i);
        gup_clk_count_init(1, CLK_AVG_TIME);

    #if 0
        gup_output_pulse2(PULSE_LENGTH);
    ...
    //-- ouput clk to GPIO 4
    buf = 0x00;
    i2c_write_bytes2(i2c_connect_client, 0x41FA, &buf, 1);
    buf = 0x00;
    i2c_write_bytes2(i2c_connect_client, 0x4104, &buf, 1);
    buf = 0x00;
    i2c_write_bytes2(i2c_connect_client, 0x4105, &buf, 1);
    buf = 0x00;
    i2c_write_bytes2(i2c_connect_client, 0x4106, &buf, 1);
    buf = 0x01;
    i2c_write_bytes2(i2c_connect_client, 0x4107, &buf, 1);
    buf = 0x06;
    i2c_write_bytes2(i2c_connect_client, 0x41F8, &buf, 1);
    buf = 0x02;
    i2c_write_bytes2(i2c_connect_client, 0x41F9, &buf, 1);
#endif
...
s32 gup_hold_ss51_dsp2(struct i2c_client *client)
{
    s32 ret = -1;
    s32 retry = 0;
    u8 rd_buf[3];
...
s32 gup_enter_update_mode_fl2(struct i2c_client *client)
{
    s32 ret = -1;
    //s32 retry = 0;
    //u8 rd_buf[3];
...
static s32 gup_burn_fw_proc(struct i2c_client *client, u16 start_addr, s32 start_index, s32 burn_len)
{
    ...
    ret = i2c_write_bytes2(client, start_addr, (u8*)&gtp_default_FW_fl2[FW_HEAD_LENGTH + start_index], burn_len);
    if (ret < 0)
    {
...
static s32 gup_check_and_repair(struct i2c_client *client, u16 start_addr, s32 start_index, s32 chk_len)
{
    ...
    while ((chk_len > 0) && (chked_times < GTP_CHK_FW_MAX))
    {
		...
		if ((FW_HEAD_LENGTH + start_index + cmp_len) > sizeof(gtp_default_FW_fl2)) {
			GTP_ERROR("Check failed, buffer overflow\n");
			break;
		}
        ret = i2c_read_bytes2(client, cmp_addr, chk_cmp_buf2, cmp_len);
        if (ret < 0)
        {
            chk_fail = 1;
            break;
		...
		for (i = 0; i < cmp_len; i++)
        {
            if (chk_cmp_buf2[i] != gtp_default_FW_fl2[FW_HEAD_LENGTH + start_index +i])
            {
                chk_fail = 1;
                i2c_write_bytes2(client, cmp_addr+i, &gtp_default_FW_fl2[FW_HEAD_LENGTH + start_index + i], cmp_len-i);
                GTP_ERROR("Check failed index: %d(%d != %d), redownload chuck", i, chk_cmp_buf2[i], 
                        gtp_default_FW_fl2[FW_HEAD_LENGTH + start_index +i]);
                break;
            }
        }
        if (chk_fail == 1)
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
    ...
    if (!memcmp(path, "update", 6))
    {
        ...
        for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i)
        {
            ...
            update_msg2.file = filp_open(FL_UPDATE_PATH, O_RDONLY, 0);
            if (IS_ERR(update_msg2.file))
            {
                update_msg2.file = filp_open(FL_UPDATE_PATH_SD, O_RDONLY, 0);
                if (IS_ERR(update_msg2.file))
                {
                    msleep(3000);
                    continue;
    ...
    update_msg2.force_update = 0xBE;     // GT9XXF ignore the 0xBE 
    if (update_msg2.fw_total_len != sizeof(gtp_default_FW_fl2))
    {
		GTP_ERROR(
				  "Inconsistent fw size. default size: %d(%dK), file size: %d(%dK)",
				  (unsigned int)sizeof(gtp_default_FW_fl2),
				  (unsigned int)sizeof(gtp_default_FW_fl2) / 1024,
				  update_msg2.fw_total_len,
				  update_msg2.fw_total_len / 1024);
        set_fs(update_msg2.old_fs);
        _CLOSE_FILE(update_msg2.file);
        return FAIL;
    }
    ...
    update_msg2.fw_total_len -= FW_HEAD_LENGTH;
    GTP_DEBUG("Fimrware size: %d(%dK)", update_msg2.fw_total_len, update_msg2.fw_total_len / 1024);
     
    update_msg2.file->f_op->llseek(update_msg2.file, 0, SEEK_SET);
    ret = update_msg2.file->f_op->read(update_msg2.file, (char*)gtp_default_FW_fl2, 
                             update_msg2.fw_total_len + FW_HEAD_LENGTH,
                                &update_msg2.file->f_pos);
    set_fs(update_msg2.old_fs);
    _CLOSE_FILE(update_msg2.file);
...
static u8 gup_check_update_file_fl(struct i2c_client *client, st_fw_head* fw_head, char* path)
{
    ...
    memcpy(fw_head, gtp_default_FW_fl2, FW_HEAD_LENGTH);
    GTP_INFO("FILE HARDWARE INFO: %02x%02x%02x%02x", fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2], fw_head->hw_info[3]);
    GTP_INFO("FILE PID: %s", fw_head->pid);
    ...
    //check firmware legality
    fw_checksum = 0;
    for(i = FW_HEAD_LENGTH; i < (FW_HEAD_LENGTH + update_msg2.fw_total_len); i += 2)
    {
        fw_checksum += (gtp_default_FW_fl2[i] << 8) + gtp_default_FW_fl2[i+1];
    }
    ret = SUCCESS;
...
s32 gup_fw_download_proc2(void *dir, u8 dwn_mode)
{
    ...
    ret = gup_enter_update_mode_fl2(i2c_connect_client);
    show_len2 = 20;
    if (FAIL == ret)
    {
