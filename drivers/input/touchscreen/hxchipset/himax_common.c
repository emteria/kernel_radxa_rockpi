...
static void himax_finger_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		GTP_SWAP(g_target_report_data->x[i],g_target_report_data->y[i]);
		g_target_report_data->x[i] = ts->pdata->abs_x_max - g_target_report_data->x[i];
		g_target_report_data->y[i] = ts->pdata->abs_y_max - g_target_report_data->y[i];
		...
		if (valid) {
			...
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X,
					g_target_report_data->x[i]);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,//panwit
					1200-g_target_report_data->y[i]);
#if !defined(HX_PROTOCOL_A)
			ts->last_slot = i;
...
static void himax_finger_leave(struct himax_ts_data *ts)
{
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		input_mt_slot(ts->input_dev, loop_i);
		input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0);
		input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, 0);//panwit
		input_report_abs(ts->input_dev, ABS_MT_PRESSURE, 0);
		input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, 0);
	}
#endif
