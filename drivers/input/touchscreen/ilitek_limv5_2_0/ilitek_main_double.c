...
static int ilitek_i2c_process_and_report(void) {
	...
	mutex_unlock(&ilitek_data->ilitek_mutex);
	//printk("zlf------\n");
	if (ilitek_wakeup_flag) {
		printk("ilitek_ts: send wakeup 111 key\n");
        rk_send_power_key(1);
		rk_send_power_key(0);
		/*input_report_key(tsdata->input, 26, 1);
        input_sync(tsdata->input);
        input_report_key(tsdata->input, 26, 0);
        input_sync(tsdata->input);*/
		ilitek_wakeup_flag=0;
	}
...
#endif
struct wake_lock chrg_lock_ilite;
