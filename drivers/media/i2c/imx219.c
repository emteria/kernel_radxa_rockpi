...
#define DST_WIDTH_3280 3280
#define DST_HEIGHT_2464 2464
#define DST_WIDTH_1920 1920
#define DST_HEIGHT_1080 1080

static int imx219_get_selection(struct v4l2_subdev *sd,struct v4l2_subdev_pad_config *cfg,struct v4l2_subdev_selection *sel) {
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct imx219 *imx219 = to_imx219(client);

	if (sel->target == V4L2_SEL_TGT_CROP_BOUNDS) {
		if (imx219->cur_mode->width == 3280) {
			sel->r.left = CROP_START(imx219->cur_mode->width, DST_WIDTH_3280);
			sel->r.width = DST_WIDTH_3280;
			sel->r.top = CROP_START(imx219->cur_mode->height, DST_HEIGHT_2464);
			sel->r.height = DST_HEIGHT_2464;
		} else {
			sel->r.left = CROP_START(imx219->cur_mode->width, imx219->cur_mode->width);
			sel->r.width = imx219->cur_mode->width;
			sel->r.top = CROP_START(imx219->cur_mode->height, imx219->cur_mode->height);
