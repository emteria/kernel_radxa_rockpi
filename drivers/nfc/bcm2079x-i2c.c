...
#define PACKET_HEADER_SIZE_RAWNCI	(3)
...
struct bcm2079x_dev {
    ...
    unsigned int tran_ver; // 2: 797 or later, 1: 795 or earlier, 0: init value
};
...
static unsigned int bcm2079x_dev_poll(struct file *filp, poll_table *wait)
{
	...
	if (bcm2079x_dev->count_irq > 0) {
        ...
        dev_info(&bcm2079x_dev->client->dev, "NFC irq count_irq=0x%x", bcm2079x_dev->count_irq);
	}
	spin_unlock_irqrestore(&bcm2079x_dev->irq_enabled_lock, flags);
				  ...
				  size_t count, loff_t *offset)
{
	...
	**/
	...
	if (total == 0 || copy_to_user(buf, tmp, total)) {
		dev_err(&bcm2079x_dev->client->dev,
			"failed to copy to user space, total = %d\n", total);
		total = -EFAULT;
	}
        //dev_info(&bcm2079x_dev->client->dev, "r:%d c:%d, 0x%02x, 0x%02x, 0x%02x", total, count, tmp[0], tmp[1], tmp[2]);
	return total;
}
				   ...
				   size_t count, loff_t *offset)
{
	...
	mutex_unlock(&bcm2079x_dev->read_mutex);
        //dev_info(&bcm2079x_dev->client->dev, "w:%d, 0x%02x, 0x%02x, 0x%02x", count, tmp[0], tmp[1], tmp[2]);
